...
function NumbersContent() {
  const biomarkers = [
    { name: "Hemoglobin", value: "14.2", unit: "g/dL", range: "13.0–17.0", normal: true },
    { name: "RBC Count", value: "5.1", unit: "M/μL", range: "4.5–5.5", normal: true },
    { name: "WBC Count", value: "7,200", unit: "/μL", range: "4,000–11,000", normal: true },
    { name: "Platelet Count", value: "245,000", unit: "/μL", range: "150,000–400,000", normal: true },
    { name: "MCV", value: "88", unit: "fL", range: "80–100", normal: true },
    { name: "Vitamin D", value: "18.4", unit: "ng/mL", range: "30.0–100.0", normal: false },
    { name: "Vitamin B12", value: "198", unit: "pg/mL", range: "200–900", normal: false },
    { name: "TSH", value: "3.8", unit: "mIU/L", range: "0.4–4.0", normal: true },
    { name: "Free T4", value: "1.2", unit: "ng/dL", range: "0.8–1.8", normal: true },
    { name: "HbA1c", value: "5.6", unit: "%", range: "4.0–5.6", normal: true },
    { name: "Fasting Glucose", value: "92", unit: "mg/dL", range: "70–100", normal: true },
    { name: "Total Cholesterol", value: "210", unit: "mg/dL", range: "< 200", normal: false },
    { name: "HDL", value: "52", unit: "mg/dL", range: "> 40", normal: true },
    { name: "LDL", value: "128", unit: "mg/dL", range: "< 100", normal: false },
    { name: "Triglycerides", value: "148", unit: "mg/dL", range: "< 150", normal: true },
    { name: "Iron", value: "72", unit: "μg/dL", range: "60–170", normal: true },
    { name: "Ferritin", value: "45", unit: "ng/mL", range: "30–400", normal: true },
    { name: "Creatinine", value: "0.9", unit: "mg/dL", range: "0.7–1.3", normal: true },
    { name: "Uric Acid", value: "6.8", unit: "mg/dL", range: "3.5–7.2", normal: true },
    { name: "ALT (SGPT)", value: "32", unit: "U/L", range: "7–56", normal: true },
  ];
  ...
  return (
    <div className="flex h-full overflow-hidden font-sans text-xs">
      ...
      <div className="w-[18%] shrink-0 bg-neutral-200/60 dark:bg-neutral-800/60 border-r border-neutral-300 dark:border-neutral-700 flex flex-col items-center gap-1.5 py-2 overflow-hidden">
        {pages.map((p) => (
          <div key={p} className="flex flex-col items-center gap-0.5">
            <div className={`w-8 h-10 rounded-[2px] border ${p === 1 ? "border-blue-500 shadow-sm shadow-blue-500/30" : "border-neutral-300 dark:border-neutral-600"} bg-white dark:bg-neutral-800 flex items-center justify-center`}>
              <div className="w-5 h-6 flex flex-col gap-[1px]">
                {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className="h-[1px] bg-neutral-300 dark:bg-neutral-600 rounded-full" />
                ))}
              </div>
            </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-white dark:bg-neutral-900 p-2">
        <div className="text-xs text-neutral-800 dark:text-neutral-200 mb-1">Complete Blood Count &amp; Metabolic Panel</div>
        <div className="text-xs text-neutral-400 mb-2">Jan 15, 2026 &middot; Thyrocare Labs</div>
        <div className="flex-1 overflow-hidden">
          ...
          {biomarkers.map((b, i) => (
            <div key={i} className="flex items-center text-xs py-[2px] border-b border-neutral-100 dark:border-neutral-800">
              <span className="flex-[3] text-neutral-700 dark:text-neutral-300 truncate">{b.name}</span>
              <span className={`flex-[2] ${b.normal ? "text-neutral-700 dark:text-neutral-300" : "text-red-500"}`}>
    ...
    <div className="flex h-full overflow-hidden font-sans text-xs">
      ...
      <div className="w-[40%] shrink-0 bg-neutral-200/60 dark:bg-neutral-800/60 border-r border-neutral-300 dark:border-neutral-700 flex flex-col overflow-hidden">
        {memos.map((m, i) => (
          ...
          >
            <div className="text-xs text-neutral-800 dark:text-neutral-200 truncate">{m.title}</div>
            <div className="text-xs text-neutral-400">{m.date} &middot; {m.duration}</div>
          </div>
        ))}
      </div>
      {/* Detail — waveform */}
      <div className="flex-1 flex flex-col min-w-0 p-2">
        ...
        <div className="flex-1 flex items-center gap-[2px]">
          {Array.from({ length: 40 }).map((_, j) => (
            <div
              key={j}
              className="flex-1 bg-blue-500/60 rounded-full"
              style={{
...
function MapsContent() {
  ...
  const dots = [
    ...
    { x: 55, y: 45 }, { x: 40, y: 55 }, { x: 60, y: 35 },
    { x: 35, y: 42 }, { x: 50, y: 60 }, { x: 58, y: 52 },
    { x: 42, y: 30 }, { x: 65, y: 48 }, { x: 38, y: 62 },
  ];
  return (
    <div className="flex h-full overflow-hidden font-sans text-xs">
      {/* Timeline sidebar */}
      ...
      <div className="flex-1 relative bg-emerald-100 dark:bg-emerald-950/40">
        ...
        {/* Current pin */}
        <div className="absolute w-3 h-3 rounded-full bg-red-500 border-2 border-white dark:border-neutral-800 shadow-sm" style={{ left: "48%", top: "48%" }} />
        {/* Zoom controls */}
        <div className="absolute bottom-2 right-2 flex flex-col rounded-md overflow-hidden shadow-sm border border-neutral-300 dark:border-neutral-600">
          <div className="w-6 h-6 bg-white dark:bg-neutral-800 flex items-center justify-center text-neutral-600 dark:text-neutral-300 text-sm border-b border-neutral-300 dark:border-neutral-600">+</div>
          <div className="w-6 h-6 bg-white dark:bg-neutral-800 flex items-center justify-center text-neutral-600 dark:text-neutral-300 text-sm">&minus;</div>
        </div>
      </div>
    </div>
  );
