...
// Pre-sort by hideAt so we can find consumed pages efficiently
const foldersByHideAt = [...folders].sort((a, b) => a.hideAt - b.hideAt);
...
export function DesktopScene() {
  ...
  for (const f of foldersByHideAt) {
    if (progress >= f.hideAt) {
      activePage = f.page;
    }
  }
  ...
  return (
    <div ref={ref} className="h-[300vh] relative">
      <div className="sticky top-0 h-dvh w-dvw bg-blue-200 dark:bg-neutral-800 flex items-center justify-center">
        ...
        <WikiWindow activePage={activePage} />
      </div>
    </div>
  );
}
