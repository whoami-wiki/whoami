...
If the extension is installed but not detected, try reinstalling it:

```bash
wai plugin install claude-code
```

For Codex or OpenCode, replace `claude-code` with `codex` or `opencode`. Make sure you restart your agent session after reinstalling.
