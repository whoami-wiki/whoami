...
export interface AuthService {
  login(username: string, password: string, ip: string | null): Promise<LoginResult>;
...
export function createAuthService(cfg: AuthServiceConfig): AuthService {
  ...
  // Real bcrypt hash of a never-matching password, computed once at startup.
  // Used as the dummy compare target when a username doesn't exist, so the
  // unauthorized-vs-not-found code paths take the same wall-clock time.
  let dummyHashPromise: Promise<string> | null = null;
  function getDummyHash(): Promise<string> {
    if (!dummyHashPromise) dummyHashPromise = hashPassword(`__no_user_${Math.random()}__`);
    return dummyHashPromise;
  }

  return {
    async login(username, password, ip): Promise<LoginResult> {
      // Rate-limit only when we have a real IP. An unidentifiable IP (no
      // X-Forwarded-For / X-Real-IP) gets no shared bucket — otherwise an
      // attacker who strips both headers locks out every other anonymous
      // caller. Tailscale ACLs are the upstream defense.
      if (ip && limiter.check(`login:${ip}`) === 'limited') {
